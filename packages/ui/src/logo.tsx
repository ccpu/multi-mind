 ...
 *
 * The same drawing is the favicon each window serves and the icon the app is
 * packaged with: `apps/desktop/public/logo.svg` is the one file both come
 * from, and `pnpm gen:icons` is what turns it into the `.png`/`.ico`/`.icns`
 * in `src-tauri/icons`. Here it is painted in `currentColor` so it follows the
 * theme, and sized like a Lucide icon so it can sit next to one without
